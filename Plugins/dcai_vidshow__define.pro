...
pro DCAI_Vidshow::frame
	...
	;image = (image - smooth(image, 80, /edge)) > 0
	...
	;\\ CROSS HAIRS
		if self.cross_hairs_on eq 1 then begin
			loadct, 0, /silent
			yc = self.cross_hairs_isect[1]
			xc = self.cross_hairs_isect[0]
			plots, [0, dims[0]]*self.zoom, [yc,yc]*self.zoom, line=1, color = 255, /device
			plots, [xc,xc]*self.zoom, [0, dims[1]]*self.zoom, line=1, color = 255, /device

			circ = findgen(361)*!DTOR
			plots, xc + self.cross_hairs_radius*sin(circ), $
				   yc + self.cross_hairs_radius*cos(circ), /device, color=255, thick = 2, line=1
		endif


	;\\ UPDATE THE FRAME RATE
		widget_control, set_value = 'Frame Rate: ' + string(dcai_global.info.frame_rate, f='(f0.2)') $
							+ ' Hz ' + ' Median: ' + string(median(*dcai_global.info.image), f='(i0)') $
							+ ' Stddev: ' + string(stddev(*dcai_global.info.image), f='(i0)'), self.info_text
...
pro DCAI_Vidshow::ViewClick, event
	...
	if event.press eq 4 and self.cross_hairs_on eq 1 then begin
		dif = ([event.x, event.y]/self.zoom)-self.cross_hairs_isect
		dif = sqrt(dif[0]*dif[0] + dif[1]*dif[1])
		self.cross_hairs_radius = dif
	endif
end
